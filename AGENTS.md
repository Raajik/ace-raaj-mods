...
- **wb_test deploy (`push wb test` / `deploy wb test` / `deploy test` full tree):** Run `bash scripts/deploy-wb-test.sh` from repo root. Same as void-test but targets **`C:\ACE\Mods\`**, stops only **`C:\ACE\`**, clears `wb_test_watchdog_*`. SQL target defaults to **`ace_world`** (`WB_TEST_SQL_DATABASE` override, `WB_TEST_SKIP_SQL=1` to skip). Same MySQL env vars as void. **Wipe overwrites `Settings.json`** under each mod — back up `C:\ACE\Mods` if needed.
...
- **Workflow — void stable first, then wb_test matches (24/7 friendly):**
  ...
  3. When void is stable and you want `C:\ACE` to match that world: **stop `C:\ACE` ACE** for the import window, then  
     `bash scripts/sync-world-void-test-to-wb-test.sh` — backs up `ace_world`, then streams **`void-test_world` → `ace_world`**.  
     Then `bash scripts/deploy-wb-test.sh` (default **`ace_world`**) so repo **`Content/SQL`** reapplies on top. Restart **`C:\ACE`**.  
     Backups: `sql-backups/world-sync/` (gitignored). `SYNC_WORLD_BACKUP_DIR`, `SYNC_WORLD_DRY_RUN=1` optional.  
     **Before import**, the sync script also writes **character/housing safety dumps** when not dry-run: full **`WB_TEST_SHARD_DATABASE`** (or `SYNC_WORLD_SHARD_DATABASE`) if set and **≠** dest world DB, plus a **subset dump** of `biota` / `character` / `house*` tables that exist **inside** the dest world DB (combined installs). `SYNC_WORLD_SKIP_PLAYER_BACKUP=1` disables those; `SYNC_WORLD_BACKUP_DEST_PLAYER_TABLES=0` skips only the subset file.
  ...
  6. **Characters / biota (no surprise wipes):** Typical ACE keeps **static world** in **`WorldDatabase`** (often `ace_world`) and **player + biota + housing runtime** in **`ShardDatabase`** (a different schema). **`sync-world-void-test-to-wb-test.sh` only targets the world DB name** (`WB_TEST_SQL_DATABASE`, default `ace_world`). A **full** clone of that world DB **does not delete characters** if your `C:\ACE` config uses that split — confirm `WorldDatabase` vs `ShardDatabase` in ACE.Server config. If both names are **the same** database, a full clone **replaces everything** in that DB (treat as destructive). Use **`SYNC_WORLD_TABLES`** (comma-separated, see script header) for a **partial** table sync, or skip cloning and run **`deploy-wb-test.sh`** alone for routine DLL + repo `Content/SQL` without bulk-replacing the whole world.
