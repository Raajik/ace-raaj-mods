...
- **void-test deploy (`push void` / `deploy void`):** Run `bash scripts/deploy-void-test.sh` from repo root. This:
  ...
  4. Always excludes ValheelContent, Shared (no csproj), build (output dir)
  - After deploy: restart the void-test ACE server to load new DLLs.
- **wb_test deploy (`push wb test` / `deploy wb test` / `deploy test` full tree):** Run `bash scripts/deploy-wb-test.sh` from repo root. Same behavior as void-test but targets **`C:\ACE\Mods\`**, stops only processes under **`C:\ACE\`** (not `C:\ACE-WB`, not void-test), clears `wb_test_watchdog_*` under `C:\ACE\Server\`. Override mod path: `WB_TEST_MODS=/path/to/Mods`. **Wipe overwrites operator-tuned `Settings.json`** in each mod folder — back up `C:\ACE\Mods` first if needed, then merge keys from repo templates.
