...
- **Workflow — void stable first, then wb_test matches (recommended):**
  1. **Pick one world database name** for both test environments (default **`void-test_world`**). In **both** ACE configs (void-test install + `C:\ACE`), set the **world** connection string / database name to that value — not `ace_world` on one and `void-test_world` on the other unless you intentionally want divergence.
  2. While iterating: `bash scripts/deploy-void-test.sh` (SQL lands on `void-test_world`; void-test Mods updated).
  3. When void is good and you want `C:\ACE` to see **the same world rows**: `export ACE_MYSQL_USER=... ACE_MYSQL_PASSWORD=...` then **`export DEPLOY_TEST_MATCH_VOID_WORLD_DB=1`** and `bash scripts/deploy-wb-test.sh`. That reapplies the **same** repo SQL files to **`void-test_world`** and copies DLLs/Meta/Settings to **`C:\ACE\Mods\`**. Restart **`C:\ACE`** ACE.Server.
  4. **Do not** run void-test and `C:\ACE` ACE **at the same time** on one shared world DB (biota writes race). Stop one, or use a **mysqldump** of `void-test_world` → restore into `ace_world` and point `C:\ACE` only at `ace_world` if you need two instances up with isolated DBs (then set `WB_TEST_SQL_DATABASE=ace_world` and treat dump+deploy as your sync step).
