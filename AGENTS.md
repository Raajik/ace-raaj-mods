...
- **wb_test deploy (`push wb test` / `deploy wb test` / `deploy test` full tree):** Run `bash scripts/deploy-wb-test.sh` from repo root. Same as void-test but targets **`C:\ACE\Mods\`**, stops only **`C:\ACE\`**, clears `wb_test_watchdog_*`. **By default** (after ACE stop, before Mods wipe) it runs **`scripts/sync-world-void-test-to-wb-test.sh`** so **`void-test_world` → `WB_TEST_SQL_DATABASE`** (default **`ace_world`**) matches void baseline, then wipes/copies mods and applies repo SQL on top. **`DEPLOY_WB_TEST_SKIP_WORLD_SYNC=1`** skips that clone (DLL/SQL-only against existing world). Requires **`ACE_MYSQL_USER`** / **`ACE_MYSQL_PASSWORD`** unless both world sync is skipped and **`WB_TEST_SKIP_SQL=1`**. SQL target defaults to **`ace_world`** (`WB_TEST_SQL_DATABASE` override). Same MySQL env vars as void. **Wipe overwrites `Settings.json`** under each mod — back up `C:\ACE\Mods` if needed.
...
- **Workflow — void stable first, then wb_test matches (24/7 friendly):**
  ...
  3. When void is stable and you want `C:\ACE` to match: run **`bash scripts/deploy-wb-test.sh`** — it **stops `C:\ACE`**, runs **`sync-world-void-test-to-wb-test.sh`** (backups under `sql-backups/world-sync/`), wipes/copies **`C:\ACE\Mods`**, reapplies repo **`Content/SQL`**. Restart **`C:\ACE`**.  
