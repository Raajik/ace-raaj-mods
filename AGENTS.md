...
- **ModData directory:** Persistent mod state (account banks, event claims, etc.) lives in `Server/ModData/<ModName>/` - this is NEVER wiped by deploy scripts, which only touch `Mods/`. The path resolves from `Assembly.GetEntryAssembly()` (ACE.Server.exe directory), NOT the plugin assembly `Assembly.Location` (which is empty for plugin-loaded assemblies).
...
| ACE `*_100_CLASS` weenies (29571+ / 36570+) = 100-unit material bags | Map via `AdditionalDepositWeenieClassIds` on the corresponding LLL DepositRule | [[ace-raaj-mods Patterns]] |
