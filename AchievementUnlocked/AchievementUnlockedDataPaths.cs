...
internal static class AchievementUnlockedDataPaths
{
    static readonly string ServerRoot = ResolveServerRoot();

    internal static string ModDataRoot => Path.Combine(ServerRoot, "ModData", "AchievementUnlocked");
    ...
    static string ResolveLegacyModRoot()
    {
        try
        {
            var assemblyPath = typeof(AchievementManager).Assembly.Location;
        ...
        return Path.GetFullPath(Path.Combine(ModManager.ModPath, "AchievementUnlocked"));
    }
