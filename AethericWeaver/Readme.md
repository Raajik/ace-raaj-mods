## AethericWeaver – Spell Customization

AethericWeaver is an ACE.BaseMod / Harmony mod that lets you reshape the spell system and equipment sets without touching core ACE data. Definitions live in an Excel spreadsheet and a JSON settings file, and are applied at world open so you can safely experiment with new spells, tweaks, and set bonuses.

`CustomSpells` contains a list of `SpellCustomization` entries that either create new spells or update existing spells at runtime.
...
1. Place `AethericWeaver.dll`, `Meta.json`, `Settings.json`, and your spreadsheet in the same mod folder (by default, the spreadsheet is `Spells.xlsx` in the mod directory).
2. Configure `Settings.json`:
   - Set `AutoloadSpreadsheet` to `true` if you want AethericWeaver to load your spreadsheet automatically when the world opens.
   - Update `Spreadsheet` if you use a different filename or path.
3. Start the server:
   - On world open, AethericWeaver loads the spreadsheet, applies each `SpellCustomization`, and then applies any set overrides in `Sets`.
4. Use the commands below to inspect and iterate on your changes (especially useful on a development shard).
