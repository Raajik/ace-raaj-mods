    ...
    public static void PostGenerateTreasure(DamageHistoryInfo killer, Corpse corpse, Creature __instance)
    {
        ...
        if (ThreadSafeRandom.Next(0.0f, 1.0f) < s.RareDropChance)
        {
            ...
            if (tool != null)
            {
                ...
                if (s.EnableDebugLogging)
                    ModManager.Log($"[BetterChestLoot] Rare drop: {tool.Name} on {__instance.Name}", ModManager.LogLevel.Info);
            }
        }
        ...
        if (ThreadSafeRandom.Next(0.0f, 1.0f) < s.RareDropChance)
        {
            ...
            if (lattice != null)
            {
                ...
                if (s.EnableDebugLogging)
                    ModManager.Log($"[BetterChestLoot] Rare drop: {lattice.Name} on {__instance.Name}", ModManager.LogLevel.Info);
            }
        }
    }
}
