...
// Applies post-GenerateTreasure adjustments to corpse loot values and quantity.
// Runtime-gated by EnableLootEconomyControl in Settings — no category gating needed.
public static class LootEconomyControl
{
    ...
    public static void PostGenerateTreasure(DamageHistoryInfo killer, Corpse corpse)
    {
        ...
        {
            ModManager.Log($"[BetterLootControl] LootEconomyControl: {items.Count} items, {valueAdjusted} values adjusted (\u00d7{valueMult}), {magicalAdjusted} magical extra-reduced (\u00d7{magicalReduction}), {capped} capped at {maxCap}p, {removedCount} removed. Corpse: {corpse.Name}", ModManager.LogLevel.Info);
        }
    }
}
