...
- **Auto-summon:** Pets spawn automatically when **you** deal damage, **you take damage**, **or your `CombatPet` deals damage** (same **1 second** pulse cooldown per owner). Harmful hostile spells you cast on creatures also pulse. Lets zero-offense / buff-only builds ramp once a device pet is hitting.
- **Lifespan:** BSS-tracked auto-summons and the extra device duplicate use **`SummoningClasses.SummonDurationSeconds`** (default **30** seconds) before `Destroy()`.
- **Visual scale:** **`SummoningClasses.AutoSummonObjScaleMultiplier`** (default **0.25**) multiplies each pet's `ObjScale` / `DefaultScale` so auto-summons render smaller; set **1** to restore full size. Restart after changing Harmony-adjacent tuning if your ops workflow requires it.
- **Move speed:** **`SummoningClasses.AutoSummonMoveSpeedMultiplier`** (default **3**) applies a postfix on `Creature.GetMovementSpeed` for **BSS-tracked** `CombatPet` instances only, multiplying `MoveSpeed` after vanilla math (so small `ObjScale` does not leave pets crawling). Set **1** to use vanilla speed for those pets.
- **Follow while moving:** When the pet has no valid attack target and you are **locomoting** (`PhysicsObj.IsMovingOrAnimating`), follow uses **`MoveFollowDistanceThreshold`** (default **6m**, max **`FollowDistanceThreshold`**) instead of the full idle leash so pets keep up. Toggle **`SummoningClasses.FollowWhileOwnerMoving`** / tune thresholds in `Settings.json`.
