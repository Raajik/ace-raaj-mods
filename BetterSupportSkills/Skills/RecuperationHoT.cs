...
// Healing kits: redirect use-on-target to self and apply a skill-scaled HoT (Recuperation; formerly Swiftmend / QOL).
...
internal static class RecuperationHoT
{
    static readonly ConcurrentDictionary<uint, RecuperationHotState> ActiveHots = new();
    ...
    static void TryApplyHealerHook(Harmony harmony)
    {
        ...
        try
        {
            ...
            if (method == null)
            {
                ModManager.Log("[BSS Recuperation] Healer.HandleActionUseOnTarget(Player, WorldObject) not found; HoT hook not applied.", ModManager.LogLevel.Error);
                return;
            }

            MethodInfo? prefix = AccessTools.Method(typeof(RecuperationHoT), nameof(PreHealerHandleActionUseOnTargetFlexible));
            if (prefix == null)
            {
                ModManager.Log("[BSS Recuperation] Flexible healer prefix method not found.", ModManager.LogLevel.Error);
                return;
            }

            harmony.Patch(method, prefix: new HarmonyMethod(prefix));
            HealerHookApplied = true;
            ModManager.Log("[BSS Recuperation] Healer use-on-target hook applied (manual Patch + __args).", ModManager.LogLevel.Info);
            LogHealerPatchesStatic();
        }
        catch (Exception ex)
        {
            ModManager.Log($"[BSS Recuperation] Failed to apply Healer hook: {ex}", ModManager.LogLevel.Error);
        }
    }
    ...
    static void TryRemoveHealerHook(Harmony harmony)
    {
        ...
        try
        {
            ...
            ModManager.Log("[BSS Recuperation] Healer use-on-target hook removed.", ModManager.LogLevel.Info);
        }
        catch (Exception ex)
        {
            ModManager.Log($"[BSS Recuperation] Failed to remove Healer hook: {ex}", ModManager.LogLevel.Error);
        }
    }
    ...
    public static bool PreHealerHandleActionUseOnTargetFlexible(Healer __instance, object[] __args)
    {
        ...
        if (cfg is null || !cfg.EnableHealing)
            return true;

        RecuperationSettings s = cfg.Recuperation;
        ...
        if (s.EnableDebugMessages)
            ModManager.Log("[BSS Recuperation] PreHealerHandleActionUseOnTargetFlexible invoked (redirect target to self).", ModManager.LogLevel.Info);

        ApplyRecuperationHot(healer, __instance, s);
        return true;
    }
    ...
    static void LogHealerPatchesStatic()
    {
        try
        {
            ...
            if (method == null)
            {
                ModManager.Log("[BSS Recuperation] Healer.HandleActionUseOnTarget not found for patch inspection.", ModManager.LogLevel.Error);
                return;
            }
            ...
            if (info?.Prefixes is { Count: > 0 })
            {
                foreach (var patch in info.Prefixes)
                {
                    ...
                    ModManager.Log($"[BSS Recuperation] Healer prefix: owner={patch.owner}, method={methodName}, priority={patch.priority}", ModManager.LogLevel.Info);
                }
            }
            else
                ModManager.Log("[BSS Recuperation] No prefixes registered on Healer.HandleActionUseOnTarget.", ModManager.LogLevel.Info);
        }
        catch (Exception ex)
        {
            ModManager.Log($"[BSS Recuperation] Error while inspecting Healer patches: {ex}", ModManager.LogLevel.Error);
        }
    }

    static void ApplyRecuperationHot(Player player, WorldObject kit, RecuperationSettings s)
    {
        try
        {
            if (player.IsDead)
            {
                ModManager.Log("[BSS Recuperation] ApplyHot: player is dead, aborting.", ModManager.LogLevel.Info);
                return;
            }
            ...
            if (skillValue <= 0)
            {
                ModManager.Log("[BSS Recuperation] ApplyHot: Healing skill <= 0, aborting.", ModManager.LogLevel.Info);
                return;
            }
            ...
            if (perTick <= 0.0)
            {
                ModManager.Log($"[BSS Recuperation] ApplyHot: perTick <= 0 (skill={skillValue}, mult={multiplier}), aborting.", ModManager.LogLevel.Info);
                return;
            }
            ...
            ModManager.Log($"[BSS Recuperation] ApplyHot: kit name='{name}', skill={skillValue}, perTick={perTick:F2}.", ModManager.LogLevel.Info);
            ...
            {
                ModManager.Log("[BSS Recuperation] ApplyHot: all per-tick values are 0, aborting.", ModManager.LogLevel.Info);
                return;
            }
            ...
            ModManager.Log($"[BSS Recuperation] ApplyHot: started/updated HoT for player {player.Name} (hp={healthPerTick}, sp={staminaPerTick}, mp={manaPerTick}, ticks={totalTicks}).", ModManager.LogLevel.Info);
            ...
            if (s.EnableDebugMessages)
            {
                ...
                player.SendMessage($"[BSS Recuperation Debug] HoT applied: +{totalHealth:F1} health, +{totalStamina:F1} stamina, +{totalMana:F1} mana over {totalTicks} ticks.", ChatMessageType.Broadcast);
            }
        }
        catch (Exception ex)
        {
            ModManager.Log($"[BSS Recuperation] ApplyRecuperationHot error: {ex}", ModManager.LogLevel.Error);
        }
    }

    static void StartTickChain(Player player, RecuperationSettings s)
    {
        if (player == null)
            return;
    ...
    static void TickOnce(Player player, RecuperationSettings s)
    {
        if (player == null)
            return;
        ...
        if (!ActiveHots.TryGetValue(id, out RecuperationHotState? state) || state == null)
        {
...
internal sealed class RecuperationHotState
{
