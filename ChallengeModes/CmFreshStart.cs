...
internal static class CmFreshStart
{
    ...
    static string BuildConfirmText(Settings s)
    {
        ...
        parts.Add("you will leave all challenge modes permanently");
        parts.Add("your character gains a small shared XP pool bonus");

        var body = parts.Count > 0 ? string.Join(", ", parts) + "." : string.Empty;
        return $"If you continue, {body} Do you wish to leave all challenge modes?";
    }
    ...
    internal static void Apply(Player player)
    {
        ...
        player.SendMessage(
            $"You have left all challenge modes and been reset to level 1. Shared enlightenment XP pool increased by {delta:0.####}.",
