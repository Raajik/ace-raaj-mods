...
// Skill credits: one per milestone per challenge mode (SSF / hardcore / alternate / aptitude), each once forever; not reset on segment.
...
public static class ChallengeMilestoneGrants
{
    ...
    public static void PrefixGrantXp(Player __instance)
    {
        ...
        if (PatchClass.Settings is not { Enabled: true, ChallengeMilestoneRewardsEnabled: true })
            return;
    ...
    public static void PostfixGrantXp(Player __instance)
    {
        ...
        var milestones = s.ChallengeMilestoneLevels;

        // One-time: old single ChallengeMilestoneClaimBits → same bits on all four tracks (no duplicate skill grants).
        ...
        if (milestones is not null && milestones.Count > 0)
        {
            for (int i = 0; i < milestones.Count; i++)
            {
                ...
                if (granted > 0)
                {
                    ...
                    __instance.SendMessage(
                        $"Challenge milestone (level {ml}): +{granted} skill credit(s) ({string.Join(", ", credited)}). One per active challenge mode, once each.");
                }
            }
        }
