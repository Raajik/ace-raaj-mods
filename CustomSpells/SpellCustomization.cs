...
public class SpellCustomization
{
    ...
    public void Apply(SpellBase sb, ACE.Database.Models.World.Spell db)
    {
        ...
        sb.Bitfield = Bitfield is not null ? sb.Bitfield | (uint)Bitfield.Value : sb.Bitfield;
    ...
    public static bool TryGetSpreadsheet(string path, out ExcelMapper excel, bool createMissing = true)
    {
        ...
        try
        {
            ...
            excel = new ExcelMapper(loadedPath);
            excel.SetupCustomSpellMappings();
        }
        catch (Exception ex)
        {
            ModManager.Log($"[CustomSpells] Failed to open spreadsheet @ {path}:\r\n{ex.Message}");
            return false;
        }
    ...
    public static void CreateDefaultSpreadsheet(string path)
    {
        ...
        ModManager.Log($"[CustomSpells] Created: {path}");
    }
    ...
    {
        var customSpells = new List<SpellCustomization>();
        ...
        {
            ModManager.Log($"[CustomSpells] Failed to parse custom spells:\r\n{ex.Message}");
        }
