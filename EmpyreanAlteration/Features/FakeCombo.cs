...
[CommandCategory(nameof(AlterationFeature.FakeCombo))]
[HarmonyPatchCategory(nameof(AlterationFeature.FakeCombo))]
public static class FakeCombo
{
    static float comboInterval = 10f;

    // Combo state resets on timer; other breakers (death, zone) may need explicit resets if extended.
