...
//[CommandCategory(nameof(AlterationFeature.FakePropertyCache))]
[HarmonyPatchCategory(nameof(AlterationFeature.FakePropertyCache))]
public static class FakePropertyCache
{
    // Long-lived caches: consider pruning on player logoff if memory becomes an issue.
    ...
    public static void PreLogOut_Final(bool skipAnimations, ref Player __instance)
    {
        ...
        cachedFloats.Remove(id);
        cachedInts.Remove(id);
    }
    #endregion
    ...
    public static void UpdateEquipmentCache(this Player player)
    {
        ...
        var caps = PatchClass.Settings?.BonusCaps;
        if (caps is null)
            return;

        foreach (var watched in watchedFloats)
        {
            if (caps.MinFloat.TryGetValue(watched, out var min) &&
                floatCache.TryGetValue(watched, out var value))
                floatCache[watched] = Math.Max(value, min);

            if (caps.MaxFloat.TryGetValue(watched, out var max) &&
        ...
        {
            if (caps.MinInt.TryGetValue(watched, out var min) &&
                intCache.TryGetValue(watched, out var value))
                intCache[watched] = Math.Max(value, min);

            if (caps.MaxInt.TryGetValue(watched, out var max) &&
    ...
    static readonly Dictionary<uint, Dictionary<FakeFloat, double>> cachedFloats = new();
    ...
    private static Dictionary<FakeFloat, double> GetOrCreateFloatCache(this Player player)
    {
        ...
        if (!cachedFloats.TryGetValue(id, out var cache))
        {
            cache = new();
            cachedFloats[id] = cache;
    ...
    static readonly Dictionary<uint, Dictionary<FakeInt, int>> cachedInts = new();
    ...
    {
        uint id = player.Guid.Full;
        if (!cachedInts.TryGetValue(id, out var cache))
        {
            cache = new();
            cachedInts[id] = cache;
    ...
    public static string DumpPlayerPropertyCache<T, E>(this Dictionary<T, E> cache) where T : notnull
    {
