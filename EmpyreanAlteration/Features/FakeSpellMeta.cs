...
//[CommandCategory(nameof(AlterationFeature.FakeSpellMeta))]
//[HarmonyPatchCategory(nameof(AlterationFeature.FakeSpellMeta))]
...
//        if (!PatchClass.Settings.SpellSettings.ReplaceBeforeCast)
//            return true;

//        //Something needs to be overridden
//        if (!(PatchClass.Settings.SpellSettings.DifferentInDungeon || PatchClass.Settings.SpellSettings.RandomizeSpells))
//            return true;

//        var spell = new Spell(spellId);

//        //Get the current spell's group
//        var comps = PatchClass.Settings.SpellSettings.GroupType switch
//        {
...
//        //Checking if a landblock has a dungeon.  Note: Replace with a proper way of seeing if position is in one
//        if (__instance.CurrentLandblock.HasDungeon && PatchClass.Settings.SpellSettings.DifferentInDungeon)
//        {
...
//        }
//        else if (PatchClass.Settings.SpellSettings.RandomizeSpells)
//        {
...
//    {
//        if (!PatchClass.Settings.SpellSettings.FistMagic)
//            return;

//        //Note: fix checking attack type instead of equipped?  //__instance?.AttackType == AttackType.Punches
...
//        if (!PatchClass.Settings.SpellSettings.FistPool.TryGetRandom(out var randomId))
//            return;
//        var spell = new Spell(randomId);

//        var powerResult = (int)(__instance.GetPowerAccuracyBar() * PatchClass.Settings.SpellSettings.FistBuckets);
//        var heightResult = ((int)__result.AttackHeight - 1) * PatchClass.Settings.SpellSettings.FistBuckets;
//        var attackBucket = powerResult + heightResult;

//        ModManager.Log($"Attacked bucket {attackBucket} ({powerResult} + {heightResult}), target {target.WeenieClassId % PatchClass.Settings.SpellSettings.TotalBuckets}");
//        if (target.WeenieClassId % PatchClass.Settings.SpellSettings.TotalBuckets == attackBucket)
...
//    {
//        //Note: Think about spell components.  If spellWords field is set I don't think it's an issue?
...
//    {
//        //if (PatchClass.Settings.SpellSettings.MetaEnabled && _metaScale.TryGetValue(__instance, out var metaPower))
//        //{
...
//        //Note: some spell version resolver
