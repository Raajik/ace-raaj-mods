...
[CommandCategory(nameof(AlterationFeature.MutatorHooks))]
[HarmonyPatchCategory(nameof(AlterationFeature.MutatorHooks))]
internal class MutatorHooks
{
    ...
    public static void SetupMutators()
    {
        ...
        foreach (var mutatorOptions in PatchClass.Settings.Mutators)
        {
    ...
    public static void ShutdownMutators()
    {
        ...
        foreach (var eventType in mutators.Values)
        {
            ...
            // Each mutator instance is shut down at most once per pass.
    ...
    public static void PostCreateAndMutateWcid(TreasureDeath treasureDeath, TreasureRoll treasureRoll, bool isMagical, ref WorldObject __result)
    {
        ...
        if (PatchClass.Settings.Verbose && mutations.Count > 0 && __result != null)
    ...
    // Prefix so mutations apply before full EnterWorld side effects; a postfix would run too late for some targets.
    [HarmonyPrefix]
    [HarmonyPatch(typeof(WorldObject), nameof(WorldObject.EnterWorld))]
