...
public class Enlightened : Mutator
{
    ...
    private static bool TryGrowItem(WorldObject item, int level, TreasureItemType_Orig itemType)
    {
        ...
        Augment augment = 0;
        //if (!PatchClass.Settings.GrowthFixedLevelAugments.TryGetValue(itemType, out var levelAugments) || !levelAugments.TryGetValue(level, out augment))
        if (!PatchClass.Settings.GrowthFixedLevelAugments.TryGetValue(item.WeenieType, out var levelAugments) || !levelAugments.TryGetValue(level, out augment))
        {
            //if (!PatchClass.Settings.GrowthAugments.TryGetValue(itemType, out var augmentGroup))
            if (!PatchClass.Settings.GrowthAugments.TryGetValue(item.WeenieType, out var augmentGroup))
                return false;

            if (!PatchClass.Settings.AugmentGroups.TryGetValue(augmentGroup, out var augmentPool))
                return false;
