...
public class Settings
{
    ...
    public string AwakenedItemUiEffectsDoc { get; init; } = "UiEffects bitmask for awakened items. Default: BoostHealth|BoostStamina = 20 (renders as red outline in AC client). Other options: Magical=1 (blue), Poisoned=2 (green), BoostHealth=4 (yellow), BoostMana=8 (blue), BoostStamina=16 (yellow), Fire=32 (red), Lightning=64 (purple), Frost=128 (white), Acid=256 (green), Nether=4096 (dark purple). Combinations are bitwise OR.";
    public uint AwakenedItemUiEffects { get; set; } = 20; // BoostHealth|BoostStamina = red outline in AC client
}
