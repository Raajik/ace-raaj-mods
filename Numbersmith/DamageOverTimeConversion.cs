...
[HarmonyPatchCategory(nameof(DamageOverTimeConversion))]
internal static class DamageOverTimeConversion
{
    ...
    public static void DeferDamage(Player player, ref float damage, bool crit, WorldObject source, DamageType damageType, SpellProjectile? spell = null)
    {
        ...
        // Integer division would drop remainder; spread extra 1 HP across first ticks so small dots are not all zero.
        var basePerTick = dot / TICKS;
        var remainder = dot - basePerTick * TICKS;
        damage -= dot;
        ...
        {
            int tickDamage = basePerTick + (i < remainder ? 1 : 0);
            actionChain.AddDelaySeconds(2);
            actionChain.AddAction(player, () =>
            {
                if (tickDamage > 0)
                    player.UpdateVitalDelta(player.Health, -tickDamage);
                player.SendMessage($"You take {tickDamage}{(crit ? " critical" : "")} delayed {damageType} damage from {name}", ChatMessageType.CombatEnemy);
            });
        }
