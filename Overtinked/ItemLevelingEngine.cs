...
// Central item-leveling logic for growth items (loot and jewelry/armor/weapons).
internal static class ItemLevelingEngine
{
    ...
    {
        // 1) Prefer imbues, then spells/cantrips, then fall back to salvage-like bonuses.
        ...
        // Spell growth will plug into this path later; for now we just ensure at least one visible effect attempt.
        return false;
    }
    ...
    private static bool TryGrantImbue(WorldObject item, Player player, int level, Settings settings, bool isArmorOrWeapon, bool emitMessages, GrowthSummary? summary)
    {
        ...
        if (currentCount >= settings.MaxImbueEffects)
            return false;
    ...
    private static bool TryApplySalvageLikeBonus(WorldObject item, Player player, int level, Settings settings, bool forWeapons, bool emitMessages, GrowthSummary? summary, bool preferJewelry = false)
    {
        ...
        List<SalvageTinkerRule> candidates = settings.SalvageRules
        ...
        SalvageTinkerRule rule = candidates[Random.Shared.Next(0, candidates.Count)];
        int value = SalvageEffectApplier.RollValue(rule);
        bool applied = SalvageEffectApplier.ApplyEffect(item, rule, value, isFailure: false);
        if (!applied)
            return false;
