...
internal static class ItemXpCurve
{
    ...
    internal static ulong TotalXpToReachItemLevel(int itemLevel, IReadOnlyList<ulong> table, int virtualCharLevel, double mult, int maxLevel)
    {
        ...
        {
            ModManager.Log($"[Overtinked] ItemXpCurve: clamping target index {targetIdx} to table max {table.Count - 1} (virtual={virtualCharLevel}, itemLevel={itemLevel}).", ModManager.LogLevel.Warn);
