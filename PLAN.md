...
- **Backlog:** WorldEvents Invasion — **Shadow Spire** disabled in `WorldEvents/Settings.json` (2026-05-04) until correct town-center `/loc` and landblock are set for the intended hub (not confused with Greenspire vs Greater Shadow Spire). On re-enable: if still Scripted, add `ace_world` `event` `GspireAttack` and matching generator/instance content as needed.
- **Backlog:** Idea routing lives under [Greenfield Backlog](#greenfield-backlog-prioritized).
- **Done (2026-05-03):** Combined delivery — AutoLoot scroll duplicate line; LLL Onyx/Obsidian salvage metadata + skeleton key appraisal suffixes; Mana Lattice 850201 (SQL + SpellSiphon self-cast + BLC bootstrap refresh). → **COMPLETED.md § 2026-05-03** “Combined”.
