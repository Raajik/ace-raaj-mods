...
[CommandCategory(nameof(Features.PetEx))]
[HarmonyPatchCategory(nameof(Features.PetEx))]
public static class PetEx
{
    // Replace factory creation of combat pets with CombatPetEx
    #region Construction
    [HarmonyPrefix]
#if REALM
...
#endif
    {
        if (weenie is null || weenie.WeenieType != WeenieType.CombatPet)
            return true;
    ...
    {
        if (biota is null || biota.WeenieType != WeenieType.CombatPet)
            return true;
...
public class CombatPetEx : CombatPet
{
    ...
    {
        if (source is not null && P_PetOwner is { Session: not null } po)
            po.SendMessage($"{Name} has been {(crit ? "critically " : "")}hit for {(int)amount} by {source.Name} {damageType} damage.", ChatMessageType.CombatEnemy);
    ...
    public override void Heartbeat(double currentUnixTime)
    {
        ...
        if (P_PetOwner is not null && (AttackTarget is null || AttackTarget.IsDestroyed))
        {
            ...
            {
#if REALM
                var sameLandblock = P_PetOwner.Location.InstancedLandblock == Location.InstancedLandblock;
#else
                var sameLandblock = P_PetOwner.Location.LandblockId == Location.LandblockId;
#endif
                if (sameLandblock)
