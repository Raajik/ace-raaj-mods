...
[CommandCategory(nameof(Features.PetExShareDamage))]
[HarmonyPatchCategory(nameof(Features.PetExShareDamage))]
public static class PetExShareDamage
{
    ...
    public static void PreTakeDamage(WorldObject source, DamageType damageType, ref float amount, bool crit, ref CombatPetEx __instance, ref uint __result)
    {
        ...
        if (player.Session != null)
            player.SendMessage($"You've been {(crit ? "critically " : "")} hit for {(int)ownerAmt} {damageType} damage by {source?.Name ?? "something"} through your link with {__instance.Name}.", ChatMessageType.CombatEnemy);
    }
}
