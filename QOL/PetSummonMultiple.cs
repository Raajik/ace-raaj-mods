...
[CommandCategory(nameof(Features.PetSummonMultiple))]
[HarmonyPatchCategory(nameof(Features.PetSummonMultiple))]
public static class PetSummonMultiple
{
    const double MAXPETS = 2;
    static Dictionary<uint, ConcurrentQueue<Pet>> playerPets { get; set; } = new();

    static decimal MaxPetWeight(this Player player)
    {
        double fromGear = 0;
        foreach (var wo in player.EquippedObjects.Values)
            fromGear += wo.GetProperty(FakeFloat.PetWeightMax) ?? 0;

        return Convert.ToDecimal(player.GetProperty(FakeFloat.PetWeightMax) ?? 1) + Convert.ToDecimal(fromGear);
    }
    ...
    static void RemovePetFromOwnerQueue(Pet destroyed)
    {
        var owner = destroyed.P_PetOwner;
        if (owner is null || !playerPets.TryGetValue(owner.Guid.Full, out var pets))
            return;

        var keep = new List<Pet>();
        while (pets.TryDequeue(out var p))
        {
            if (p is null || p.Guid == destroyed.Guid)
                continue;
            if (p.IsAlive)
                keep.Add(p);
        }
        ...
        owner.SendMessage($"{destroyed.Name} shuffled off this mortal coil.");
    }
    ...
    public static bool PreInit(Player player, PetDevice petDevice, ref Pet __instance, ref bool? __result)
    {
        ...
        if (!playerPets.TryGetValue(player.Guid.Full, out var pets))
        {
            pets = new();
            playerPets.TryAdd(player.Guid.Full, pets);
        }
    ...
    //    {
    //        // Alternate: rebuild list excluding dead pet (PostDestroy path uses RemovePetFromOwnerQueue instead).
    ...
    public static void Sim(Session session, params string[] parameters)
    {
        ...
        if (!playerPets.TryGetValue(player.Guid.Full, out var pets) || pets.Count == 0)
        {
        ...
        switch (verb)
        {
            ...
            case PetCommand.Warp:
                foreach (var pet in pets)
                {
                    if (pet is null || !pet.IsAlive)
                        continue;

                    if (player.GetCylinderDistance(pet) > 5)
                    {
                        if (pet.TryTeleport(player.Location.InFrontOf(1), out _))
