...
**Cross-event systems:**
- **Pending event rewards** — Loot queued while offline lives under `Mods/WorldEvents/PendingClaims/`. **Login:** System reminder if pending (no auto-grant). **Optional** periodic auto-claim: `/claim auto on` (default **off**); preference saved under `Mods/WorldEvents/PendingClaimsAuto/<characterGuid>.json` (persists across logins). Every **120 minutes** while online, opted-in characters get an auto `/claim` attempt and a nudge if anything remains queued.
