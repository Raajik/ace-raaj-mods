...
1. Build a mod: `dotnet build ModName/ModName.csproj` (outputs under repo `build/<ModName>/` unless your `.csproj` overrides `OutputPath`).
...
Local builds write DLLs and copied `Meta.json` / `Settings.json` to **`build/<ModFolderName>/`** at the repo root (see each `.csproj` for `OutputPath` / post-build copy targets).
...
### Full-tree deploy scripts (Git Bash)

From repo root **`A:\ai\projects\ace-raaj-mods`** (Git Bash paths: `/a/ai/projects/ace-raaj-mods`):
