...
4. **Examine** — types **14** (Use), **15** (ShortDesc), **16** (LongDesc): state **bulk turn-in at Trophy Collector**, **XP + banked trade notes** (Ley Line / `BankCashProperty`), not vanilla pocket pyreals. Drudge Bloodletter line uses the **same** reward sentence on **14, 15, and 16** (operator choice). If the base weenie has no rows for 14–16, use **`INSERT ... ON DUPLICATE KEY UPDATE`** (see `DrudgeCharm_TierWeenies_World.sql`) so clones and base all get strings.
5. **UI** — `weenie_properties_int` type **179** (`ImbuedEffect`) + **18** (`UiEffects`) for per-tier client rim/glow on stackables (see `ACE-Item-Icon-UiEffects-ImbuedEffect-Reference.md` drudge table). Type **131** is `MaterialType` (different).
...
9. **Apply** to `ace_world`, **restart** ACE (weenie cache). Optional: `mysqldump` scoped slice before apply per `AGENTS.md`. Forward-only fixes: **`DrudgeCharm_BloodletterPerTierRestore_2026-05-07.sql`**. Deprecated stubs: `DrudgeCharm_SunstoneRimAndCopy_2026-05-06.sql`, `DrudgeCharm_BloodletterBase_2026-05-05.sql`, `DrudgeCharm_UiRendIcons_2026-05-05.sql`.
