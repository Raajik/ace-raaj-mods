...
5. **UI** — `weenie_properties_int` type **18** (`PropertyInt.UiEffects`, bitmask) for glow/outline; type **179** (`ImbuedEffect`) for imbue-style icon chrome. Drudge Bloodletter line uses **per-tier** 179/18 pairs (see reference doc), not a single sunstone uniform. Type **131** is `MaterialType` (different).
...
9. **Apply** to `ace_world`, **restart** ACE (weenie cache). Optional: `mysqldump` scoped slice before apply per `AGENTS.md`. Forward-only fixes: **`DrudgeCharm_BloodletterBase_2026-05-05.sql`** (supersedes deprecated sunstone filename stub).

## Reference implementation

- `WindblownContent/Content/SQL/DrudgeCharm_TierWeenies_World.sql` — drudge charms **24835** (Bloodletter base), **850271–850273** (clone tiers from 24835).
