...
**Examine copy:** Only `weenie_properties_string` **type 16 (LongDesc)** carries the line *"Collectors will reward a great deal of experience and pyreals for turning in these charms."* Types **14** (Use) and **15** (ShortDesc) are deleted — populating both 14 and 16 makes the client render the sentence twice in the examine panel. (Updated **2026-05-04**: previous wording mentioned "Collectors and Trophy Collectors"; only **Collectors** actually accept these charms — Trophy Collectors do not have a Give emote chain for the drudge charm WCIDs and there's no plan to add one.)
