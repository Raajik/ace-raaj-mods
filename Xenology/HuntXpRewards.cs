...
internal static class HuntXpRewards
{
    internal static bool TryGrantPlacementXp(Settings settings, int zeroBasedRank, Player player, out long amount, out string formulaSummary)
    {
        ...
        formulaSummary = $"{pct:0.##}% of XP to next level ({label} — HuntPlacementXpFraction{FractionSuffix(zeroBasedRank)})";

        player.SendMessage($"[Xenology] Hunt placement XP ({PlacementLabel(zeroBasedRank)}): +{awarded:N0} character XP ({pct:0.##}% of XP to next level).");
        return true;
    }
