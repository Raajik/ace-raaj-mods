...
$watchdog = "C:\ACE\Server\WbTestWatchdog.ps1"

if (-not (Test-Path $watchdog))
{
    throw "Missing watchdog script: $watchdog — create it first or deploy from repo."
}
...
$regArgs = @{
    ...
    Settings    = $settings
    Principal   = $principal
    Description = "Runs WbTestWatchdog.ps1 as interactive user: restarts C:\ACE\Server\ACE.Server.exe if absent. ACE console visible on desktop."
}
