...
#   Optional: MYSQL_EXE (interactive client; default MySQL 8.0 under Program Files on Windows),
#     MYSQLDUMP_EXE (defaults to mysqldump next to mysql — required for backups/stream dump).
...
if [ -z "$MYSQLDUMP_EXE" ]; then
  _m_dir="$(dirname -- "$MYSQL_EXE")"
  _m_base="$(basename -- "$MYSQL_EXE")"
  case "$_m_base" in
    mysql.exe|mysql.EXE|MYSQL.EXE)
      if [ -x "$_m_dir/mysqldump.exe" ]; then
        MYSQLDUMP_EXE="$_m_dir/mysqldump.exe"
      elif [ -x "/c/Program Files/MySQL/MySQL Server 8.0/bin/mysqldump.exe" ]; then
        MYSQLDUMP_EXE="/c/Program Files/MySQL/MySQL Server 8.0/bin/mysqldump.exe"
      elif [ -x "/mnt/c/Program Files/MySQL/MySQL Server 8.0/bin/mysqldump.exe" ]; then
        MYSQLDUMP_EXE="/mnt/c/Program Files/MySQL/MySQL Server 8.0/bin/mysqldump.exe"
      else
        MYSQLDUMP_EXE="$_m_dir/mysqldump.exe"
      fi
      ;;
    *)
      if [ -x "$_m_dir/mysqldump" ]; then
        MYSQLDUMP_EXE="$_m_dir/mysqldump"
      else
...
if ! command -v "$MYSQLDUMP_EXE" &>/dev/null && ! [ -x "$MYSQLDUMP_EXE" ]; then
  echo "ERROR: mysqldump not found: $MYSQLDUMP_EXE (set MYSQLDUMP_EXE)" >&2
  exit 1
fi
...
"$MYSQLDUMP_EXE" --defaults-extra-file="$CNF_CLI" --default-character-set=utf8mb4 \
...
if [ "${SYNC_WORLD_DRY_RUN:-0}" = "1" ]; then
  ...
  if [ "${#SYNC_TABLE_ARGS[@]}" -gt 0 ]; then
    "$MYSQLDUMP_EXE" "${DUMP_BASE_FLAGS[@]}" "$SOURCE_DB" "${SYNC_TABLE_ARGS[@]}" \
      | sed "$sed_rewrite" >"$DUMP_ONLY"
  else
    "$MYSQLDUMP_EXE" "${DUMP_BASE_FLAGS[@]}" "$SOURCE_DB" \
      | sed "$sed_rewrite" >"$DUMP_ONLY"
  fi
